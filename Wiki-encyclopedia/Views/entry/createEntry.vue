  ...
  <div style="display: flex;width: 1280px;margin: 0 auto 20px">
    <div style="width: 900px;display: flex;flex-direction: column">
      <div>
        ...
        <el-input v-model="entryName" maxlength="20"></el-input>
      </div>
      <!-- 词条分类 -->
      <div class="mg-top-20">
        ...
        <div class="block-container">
          <el-tag v-for="item in savedCategories" :key="item.id" type="primary">{{ item.name }}</el-tag>
          <el-button
            class="button-new-category"
            size="small"
            @click="showCategoryChooseModel"
          >+ 添加分类</el-button>
        </div>
      </div>
      <!-- 同义词 -->
              ...
              <div v-if="!item.addBySelf">
                ...
                <span style="position: relative;" v-if="item.attributeType === 2">
                  ...
                  <span
                    ...
                    v-show="item.noValid"
                  >属性值在{{ item.attributeRangeBegin }}～{{ item.attributeRangeEnd }}之间</span>
                </span>
                <!--枚举-->
      ...
      <!--              <p>-->
      <!--                网址:<a target="_blank" class="quote-btn" @click="goLink(item.url)">{{-->
      <!--                  item.url-->
      <!--                }}</a>-->
      <!--              </p>-->
      <!--            </el-card>-->
    ...
    <div>
      <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="外部词条模板" name="outer">
          <el-tree
            class="template-left"
                  ...
                  <ul>
                    <li v-for="el in item['children']" v-bind:key="el.id">
                      ...
                      <ul class="thrid-class">
                        <li v-for="e in el['children']" v-bind:key="e.id">{{ e.contentName }}</li>
                      </ul>
                    </li>
                  </ul>
                </template>
        ...
        <el-tab-pane label="内部词条模板" name="inner" v-if="permission === '0'">
          <el-tree
            class="template-left"
            :data="innerTreeList"
            :props="{ label: 'name' }"
        ...
        <el-tab-pane label="词条目录" name="menu">
    ...
    <el-dialog title="选择分类" :visible.sync="dialogVisible" :before-close="handleCloseModel">
      <tree-transfer
        ...
        height="540px"
        :title="title"
        :from_data="entryType === '0' ? innerTreeList : categoryTreeData"
        :to_data="toData"
...
import { getAllAttributesByCategoryId,findEntry,updateEntry,deleteEntry,relatedEntry,getEntryAll } from '@/api/classifyManager'
...
export default {
  ...
  data() {
    return {
      ...
      myConfig: {
        ...
        // serverUrl: 'http://jy5eca.natappfree.cc/wiki-backend/api/ueditor/config'
      ...
      categoryTreeData: [], // 外部词条模板目录
      innerTreeList: [], // 内部词条模板目录
      toData: [],
      savedCategories: [],
  ...
  methods: {
  getEntryAll(){
    getEntryAll({entryName: this.searchEntry})
        .then(res => {
          console.log(res)
        })
        .catch(error => {
  ...
  relatedEntry(){
    ...
    watchNumber(item) {
      ...
      if (item.val !== '' && (val > item.attributeRangeEnd || val < item.attributeRangeBegin)) {
    ...
    commit(method) {
      ...
      this.classifyData.map(item => {
        ...
        if (item.val && item.val !== '') {
          attr =
            ...
            item.attributeName +
            '</dt><dd class="basicInfo-item value">' +
            item.val +
            '</dd>'
          // attributesAry.push({
      ...
      attributesAry = '<dl class="basicInfo-block basicInfo-left">' + attr + '</dl>'
      if (noValid) {
      ...
      if (this.msg.indexOf('<p') !== -1) {
        this.msg = this.msg.replace(/<p/g, '<p class="para"')
      }
      ...
      let newContent = this.msg + attributesAry + this.msg1
      let data = {
        operate: method,
        editReson: '',
        entryId: '', // 返回值
      ...
      vm.$axios.post('/wiki-backend/api/entry/save', data).then(res => {
    ...
    getCategoryTree() {
      categoryApi
        ...
        .then(res => {
          if (res.status === 'success') {
            this.categoryTreeData = res.data && (_.cloneDeep(res.data.children) || [])
            this.getInnerTree()
          } else {
    ...
    getInnerTree() {
      getInternalEntryList()
        .then(res => {
          if (res.status === 'success') {
            this.innerTreeList = res.data && (_.cloneDeep(res.data.children) || [])
          } else {
            this.$message.error('获取分类信息失败，请稍后重试！')
          }
        })
    ...
    // 限制最多选5个
    checkLength(nodeObj, treeObj) {
      let treeComp = this.$refs.treeTransfer.$children[2],
        arr = treeComp.getCheckedNodes().filter(x => !x.children.length)
      if (arr && arr.length + this.savedMiddleCategories.length > 5) {
        ...
        this.$refs.treeTransfer.from_check_keys = ['1']
      }
    },
    // 关闭弹窗二次确认
