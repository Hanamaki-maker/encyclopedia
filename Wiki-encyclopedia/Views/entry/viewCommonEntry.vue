<template>
  <div
    style="display: flex;margin: 0 auto;max-width: 1920px;width: 99%;background: #f0f0f0;"
    id="entry-container"
    v-if="!doReload"
    ...
    <div style="width: calc(100% - 0px);display: flex;flex-direction: column;margin-bottom: 50px">
      ...
      <div v-show="seeMore">
        ...
        <div
          class="mg-top-20"
          id="attribute"
          style="display: flex;flex-wrap: wrap;padding: 20px 0;margin-top: 50px;"
        ...
        <div class="mg-top-20" style="display: flex;flex-direction: row" id="catalogue">
          <div
            ...
            style="width: 97px;display: inline-block;background: #fbfbfb;text-align: center;position: relative;"
          >
            ...
            <template v-for="(item, index) in wikiContent.entryContentVos">
              <div :key="index">
                <h2
                  ...
                  :id="item.id"
                  v-if="item.contentTitle !== null && item.contentTitle !== 'null'"
                >
                  {{ item.contentTitle }}
                </h2>
                ...
                <template v-if="item.children.length">
                  <div class="text-indent" v-for="(key, index) in item.children" :key="index">
                    <h3
                      :id="key.id"
                  ...
                  <template v-if="key.children.length">
                    <div class="text-indent" v-for="(v, index) in key.children" :key="index">
          ...
          <div id="rightNav" class="rightNav" style="max-height: 310px;overflow-y: scroll">
            ...
            >
              ...
              <div v-for="(k, index1) in item.children" :key="index1">
                ...
                <template v-for="(item, index) in k.children">
                  <a
                    :key="index"
...
export default {
  ...
  mounted() {
    ...
    if (vm.viewType === 'preview') {
      ...
      this.$axios.post('/wiki-backend/api/entry/info', { id: vm.entryId }).then(res => {
        ...
        setTimeout(() => {
          for (let i = 0; i < target.length; i++) {
            if (target[i].hasAttribute('data-original')) {
              target[i].src =
                this.baseUrlConfig.IMG_PREFIX + target[i].getAttribute('data-original')
            }
          }
