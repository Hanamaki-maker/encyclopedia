...
<!--          <el-tag v-for="item in savedCategories" :key="item && item.id" type="primary">-->
<!--            {{ item && item.name }}-->
<!--          </el-tag>-->
    ...
    <div>
      <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="修改目录" name="second">
          <div v-for="(item, index1) in menuList" :key="index1">
            <p class="p1">{{ item.title }}</p>
            <div v-for="(k, index2) in item.children" :key="index2">
              <p class="p2">{{ k.title }}</p>
              <p class="p3" v-for="(v, index3) in k.children" :key="index3">{{ v.title }}</p>
            </div>
          </div>
        </el-tab-pane>
      </el-tabs>
...
import { categoryTree, getAllAttributesByCategoryId, getInternalEntryList } from '@/api/classifyManager/index.js'
...
export default {
  ...
  data() {
    return {
      ...
      myConfig: {
        ...
        // serverUrl: 'http://192.168.1.82:8080/wiki-backend/upload2/upload'
  ...
  watch: {
    savedCategories(newValue) {
      this.savedCategoriesArr =
        newValue && newValue.map(x => ({ categoryId: x && x.id ? x.id : '' }))
    }
  },
  // updated() {
  //   this.$nextTick(() => {
  ...
  //         if (
  //           target[i].hasAttribute('src') &&
  //           target[i].getAttribute('src').indexOf(this.baseUrlConfig.IMG_PREFIX) === -1
  //         ) {
    ...
    Cetc10Auth().init(function() {
      ...
      vm.$axios
        ...
        .then(res => {
          ...
          // data.entryContentVos &&
          //   data.entryContentVos.map(item => {
          //     let obj1 = {
          //       title:
          //         item.contentTitle === 'null' || item.contentTitle == null
          //           ? ''
          //           : item.contentTitle,
          //       content: item.contentBody === '<p>null</p>' ? '<p>&nbsp</p>' : item.contentBody,
          //       children: []
          //     }
          //     vm.msg1 = vm.msg1 + '<h2>' + obj1.title + '</h2>' + obj1.content
          //     item.children.map(k => {
          //       let obj2 = {
          ...
          //       vm.msg1 = vm.msg1 + '<h3>' + obj2.title + '</h3>' + obj2.content
          //       k.children.map(v => {
          //         let obj3 = {
          //           title:
          //             v.contentTitle === 'null' || v.contentTitle === null ? '' : v.contentTitle,
          //           content: v.contentBody === '<p>null</p>' ? '<p>&nbsp</p>' : v.contentBody
          //         }
          //         obj2.children.push(obj3)
          //         vm.msg1 = vm.msg1 + '<h4>' + obj3.title + '</h4>' + obj3.content
          //       })
          //     })
          //     vm.model.push(obj1)
          //   })
          data.entryReferrences &&
  ...
  methods: {
    ...
    commit(method) {
      ...
      attributesAry = JSON.stringify(attributesAry);
      // 摘要p标签添加类名
      ...
      let contentBody = {
        title: '',
        children: [],
        contentBody: this.msg1
      }
      let newContent = JSON.stringify(contentBody)
      let summaryData = {
      ...
      newContent = summary + '*' + attributesAry + '*' + newContent;
      let data = {
        operate: method,
        editReson: '',
    ...
    getCategoryTree() {
      ...
      categoryApi
        ...
        .then(res => {
          if (res.status === 'success') {
            vm.categoryTreeData = res.data && (_.cloneDeep(res.data.children) || [])
          } else {
            this.$message.error('获取分类信息失败，请稍后重试！')
          }
        })
    ...
    saveCategory() {
      ...
      this.savedCategoriesArr = this.savedCategories.map(x => ({ categoryId: x.id }))
      this.savedCategoriesKeysAry = this.savedCategories.map(x => x.id)
